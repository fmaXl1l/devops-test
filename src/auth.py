...
def validate_api_key(x_parse_rest_api_key: str = Header(None, alias="X-Parse-REST-API-Key")) -> str:
    """
...
def validate_jwt_token(x_jwt_kwy: str = Header(None, alias="X-JWT-KWY")) -> dict:
    ...
    try:
        ...
        if "transaction_id" not in payload:
            raise HTTPException(status_code=401, detail="Invalid JWT: missing transaction_id")
        return payload
